...
// --- LIVE EXCHANGE RATE FETCHER (GARANTI BBVA BANK SATIŞ KURU ENTEGRASYONU) ---
  ...
  try {
    ...
    if (resUsd && resUsd.ok) {
      ...
      if (dataUsd && dataUsd.rates && dataUsd.rates.TRY) {
        ...
        const garantiUsdSell = baseUsd * 1.008;
        exchangeRates.USD = parseFloat(garantiUsdSell.toFixed(2));
      }
    }
    ...
    if (resEur && resEur.ok) {
      ...
      if (dataEur && dataEur.rates && dataEur.rates.TRY) {
        ...
        exchangeRates.EUR = parseFloat(garantiEurSell.toFixed(2));
      }
    }
    ...
    console.warn('Canlı döviz kuru güncelleme uyarısı:', err);
  } finally {
    if (syncBtn) {
...
function updateItemKdv(index, val) {
  if (currentQuoteItems[index]) {
    currentQuoteItems[index].kdv = parseFloat(val) || 0;
    updateCalculations();
    updatePdfPreview();
  }
}
...
function updateCalculations() {
  ...
  currentQuoteItems.forEach((item, idx) => {
    ...
    tr.innerHTML = `
      ...
      <td>
        <select class="form-control" style="width: 75px; padding: 4px;" onchange="updateItemKdv(${idx}, this.value)">
          <option value="20" ${item.kdv == 20 ? 'selected' : ''}>%20</option>
          <option value="10" ${item.kdv == 10 ? 'selected' : ''}>%10</option>
          <option value="0" ${item.kdv == 0 ? 'selected' : ''}>%0</option>
        </select>
      </td>
      <td>
...
function updatePdfPreview() {
  ...
  const paymentInput = document.getElementById('terms-payment');
  const payment = paymentInput ? paymentInput.value : '';
  const deliveryInput = document.getElementById('terms-delivery');
  ...
  const pdfNo = document.getElementById('pdf-val-no'); if (pdfNo) pdfNo.textContent = quoteNo;
  const pdfDate = document.getElementById('pdf-val-date'); if (pdfDate) pdfDate.textContent = formattedDate;
  const pdfValidity = document.getElementById('pdf-val-validity'); if (pdfValidity) pdfValidity.textContent = validity;
  
  const pdfCompany = document.getElementById('pdf-val-company'); if (pdfCompany) pdfCompany.textContent = company;
  const pdfContact = document.getElementById('pdf-val-contact'); if (pdfContact) pdfContact.textContent = contact;
  const pdfPhone = document.getElementById('pdf-val-phone'); if (pdfPhone) pdfPhone.textContent = phone;
  const pdfEmail = document.getElementById('pdf-val-email'); if (pdfEmail) pdfEmail.textContent = email;
  const pdfTax = document.getElementById('pdf-val-tax'); if (pdfTax) pdfTax.textContent = tax;
  const pdfAddress = document.getElementById('pdf-val-address'); if (pdfAddress) pdfAddress.textContent = address;
  const pdfSigCustomer = document.getElementById('pdf-sig-customer-name'); if (pdfSigCustomer) pdfSigCustomer.textContent = company;

  const pdfCurrency = document.getElementById('pdf-val-currency'); if (pdfCurrency) pdfCurrency.textContent = `${activeCurrency} (${getCurrencySymbol()})`;
  const pdfPayment = document.getElementById('pdf-val-payment'); if (pdfPayment) pdfPayment.textContent = payment;
  const pdfDelivery = document.getElementById('pdf-val-delivery'); if (pdfDelivery) pdfDelivery.textContent = delivery;
...
async function saveAndExportPdf() {
  ...
  // Create fixed size offscreen A4 container (794px x 1123px = 210mm x 297mm at 96dpi)
  ...
  try {
    ...
    const pdf = new html2pdf.jsPDF('p', 'mm', 'a4');
    pdf.addImage(imgData, 'JPEG', 0, 0, 210, 297);
    pdf.save(filename);

    if (document.body.contains(container)) document.body.removeChild(container);
    alert(`"${filename}" tek sayfa A4 formatında kusursuz olarak oluşturuldu ve indirildi!`);
    renderDashboardMetrics();

  } catch (err) {
    if (document.body.contains(container)) document.body.removeChild(container);
    console.error('PDF Generation Error:', err);
    alert('PDF oluşturulurken bir hata oluştu. Lütfen "Doğrudan Yazdır" butonunu kullanınız.');
  }
}
