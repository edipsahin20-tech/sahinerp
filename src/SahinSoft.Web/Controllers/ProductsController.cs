...
public sealed class ProductsController(
    ...
    StockCodeGeneratorService stockCodeGenerator,
    IWebHostEnvironment webHostEnvironment) : Controller
{
    // Yüklenen ürün fotoğrafları wwwroot/uploads/products altında saklanır - bu klasör hiçbir
    // zaman dotnet publish çıktısının bir parçası olmadığı için Kurulum.ps1'in Expand-Archive
    // -Force ile üzerine yazdığı yayın paketinden etkilenmez, yeniden dağıtımda kaybolmaz.
    private const string ProductImageUploadRelativePath = "uploads/products";
    private static readonly string[] AllowedImageExtensions = [".jpg", ".jpeg", ".png", ".webp"];
    private const long MaxImageBytes = 5 * 1024 * 1024;

    private async Task<(bool Ok, string? ImagePath)> ResolveProductImageAsync(ProductFormViewModel form, string? existingImagePath)
    {
        if (form.RemoveImage)
        {
            DeleteProductImageFileIfLocal(existingImagePath);
            return (true, null);
        }

        if (form.ImageFile is not { Length: > 0 } file)
        {
            return (true, existingImagePath);
        }

        var extension = Path.GetExtension(file.FileName).ToLowerInvariant();
        if (!AllowedImageExtensions.Contains(extension))
        {
            ModelState.AddModelError(nameof(ProductFormViewModel.ImageFile), "Sadece JPG, PNG veya WEBP formatında görsel yüklenebilir.");
            return (false, existingImagePath);
        }
        if (file.Length > MaxImageBytes)
        {
            ModelState.AddModelError(nameof(ProductFormViewModel.ImageFile), "Görsel boyutu 5 MB'ı geçemez.");
            return (false, existingImagePath);
        }

        var uploadsDir = Path.Combine(webHostEnvironment.WebRootPath, ProductImageUploadRelativePath);
        ...
        DeleteProductImageFileIfLocal(existingImagePath);
        return (true, $"/{ProductImageUploadRelativePath}/{fileName}");
    }

    // Eski dosyanın diskten silinmesi en iyi çaba (best-effort) - başarısız olursa (kilitli dosya
    // vb.) kayıt işlemini bozmamalı, sadece kullanılmayan bir dosya diskte kalır.
    private void DeleteProductImageFileIfLocal(string? imagePath)
    {
        if (string.IsNullOrWhiteSpace(imagePath) || !imagePath.StartsWith($"/{ProductImageUploadRelativePath}/", StringComparison.OrdinalIgnoreCase))
        {
            return;
        }
    ...
    private async Task<IActionResult> CreateCoreAsync(ProductFormViewModel form)
    {
        ...
        var (imageOk, resolvedImagePath) = await ResolveProductImageAsync(form, null);
        if (!ModelState.IsValid || !imageOk)
        {
            await PopulateSelectionsAsync(form);
            return View("Form", form);
        }
        // MapAsync birden fazla kez çağrılabilir (eşzamanlılık nedeniyle stok kodu/barkod yeniden
        // üretme döngüsü) - dosya zaten kaydedildi, form.ImagePath'i sabitleyip ImageFile'ı
        // temizliyoruz ki her turda aynı sonuç kullanılsın, dosya tekrar okunmaya çalışılmasın.
        form.ImagePath = resolvedImagePath;
        form.ImageFile = null;
    ...
    public async Task<IActionResult> Edit(int id, ProductFormViewModel form)
    {
        ...
        var (imageOk, resolvedImagePath) = await ResolveProductImageAsync(form, product.ImagePath);
        if (!ModelState.IsValid || !imageOk)
        {
            await PopulateSelectionsAsync(form);
            return View("Form", form);
